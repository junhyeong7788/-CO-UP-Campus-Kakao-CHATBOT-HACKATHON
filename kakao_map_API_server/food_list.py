...
driver.get(url)
key_word = '대구 범어동 음식점'  # 검색어

# css 찾을때 까지 10초대기
def time_wait(num, code):
    try:
        wait = WebDriverWait(driver, num).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, code))) #EC.presence_of_element_located : 해당 태그가 나올때까지 기다림
    except:
        print(code, '태그를 찾지 못하였습니다.') 
        driver.quit()
    return wait # 찾은 태그를 반환

# 음식점 정보 출력
def food_list_print(): # 음식점 리스트 출력

    time.sleep(0.2)

		# (3) 장소 목록
    food_list = driver.find_elements(By.CSS_SELECTOR, '.placelist > .PlaceItem') # placelist : 장소목록, PlaceItem : 장소목록의 각 항목
    ...
    for index in range(len(food_list)):
        ...
        food_name = names[index].text
        print(food_name)

        food_type = types[index].text
        print(food_type)
        ...
        food_dict['음식정보'].append(dict_temp)
...
# json 파일로 저장
with open('/Users/jun/Downloads/food_data.json', 'w', encoding='utf-8') as f:
